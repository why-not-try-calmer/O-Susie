...
def create_verification_keyboard() -> types.InlineKeyboardMarkup:
    ...
    keyboard: List[List[InlineKeyboardButton]] = rows_of_3(buttons)
...
class Verify:
    ...
    def can_verify(chat_id: int, user_id: int) -> bool:
        if (
            not chat_id in Verify.chats
            or not user_id in Verify.chats[chat_id].users
            or Verify.chats[chat_id].users[user_id].status
            != Status.challenged_to_verify
        ):
            return False
        return True
    ...
    def has_last_chance(chat_id: int, user_id: int) -> bool:
        if (
            Verify.chats[chat_id].users[user_id].attempts == 0
            and datetime.now() - Verify.chats[chat_id].users[user_id].joined_at
            < config["delta"]
        ):
    ...
    async def authorize(chat: types.Chat, user_id: int) -> None:
        ...
        await asyncio.gather(
            *[
                chat.delete_message(t)
                for t in Verify.chats[chat.id].users[user_id].pending_messages_ids
            ]
        )
    ...
    def schedule_reject(bot: Bot, chat: types.Chat, user_id: int) -> None:
        async def kicking() -> None:
            ...
            reply = try_sending_message(
                bot, chat_id=chat.id, text=f"Time elapsed, kicked user {user_id}"
            )
